...
let NeuralNetworkDetection = require('./NeuralNetworkDetection.js');
let std_msgs = _finder('std_msgs');

//-----------------------------------------------------------

class NeuralNetworkDetectionArray {
  constructor(initObj={}) {
    if (initObj === null) {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.detections.forEach((val) => {
      bufferOffset = NeuralNetworkDetection.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new NeuralNetworkDetectionArray(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.detections[i] = NeuralNetworkDetection.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.detections.forEach((val) => {
      length += NeuralNetworkDetection.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static datatype() {
    ...
    return 'neural_network_msgs/NeuralNetworkDetectionArray';
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new NeuralNetworkDetectionArray(null);
    ...
    if (msg.detections !== undefined) {
      ...
      for (let i = 0; i < resolved.detections.length; ++i) {
        resolved.detections[i] = NeuralNetworkDetection.Resolve(msg.detections[i]);
      }
    }
    else {
      resolved.detections = []
...
module.exports = NeuralNetworkDetectionArray;
